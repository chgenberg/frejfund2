...
export function generateProactiveInsights(businessInfo: BusinessInfo): string[] {
  ...
  if (
    businessInfo.stage === 'growth' &&
    (!businessInfo.monthlyRevenue || parseInt(businessInfo.monthlyRevenue) < 50000)
  ) {
    insights.push(
      '⚠️ Your revenue seems low for a growth-stage company. Most investors expect $50K+ MRR at this stage.',
    );
  }
  ...
  if (businessInfo.teamSize === '1' && businessInfo.stage !== 'idea') {
    insights.push(
      '⚠️ Solo founders often struggle to raise funding. Consider finding a co-founder or explaining why you can execute alone.',
    );
  }
  ...
  if (gaps.length > 0) {
    insights.push(
      `📊 I need more data to properly assess your investment readiness. Missing: ${gaps[0].missing.join(', ')}`,
    );
  }
  ...
  if (businessInfo.monthlyRevenue && parseInt(businessInfo.monthlyRevenue) > 10000) {
    insights.push(
      '✅ Great to see you have revenue traction! This significantly improves your fundraising chances.',
    );
  }
...
export function getNextBestAction(businessInfo: BusinessInfo): {
  ...
  if (
    ...
    businessInfo.monthlyRevenue &&
    parseInt(businessInfo.monthlyRevenue) > 0
  ) {
    return {
