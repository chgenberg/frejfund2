...
const CATEGORY_BATCHES: { category: string; prompt: (bi: BusinessInfo) => string }[] = [
  {
    ...
    prompt: (bi) =>
      `Summarize known public info about ${bi.companyName || bi.name || 'the company'}: founding year, HQ/country, founders, product summary, market segment, pricing level, notable customers, funding history, and press mentions. Return compact bullet points.`,
  },
  {
    category: 'Market & Competition',
    prompt: (bi) =>
      `For ${bi.companyName || bi.name || 'the company'}, summarize market size signals, competitors, differentiators, and timing. Return concise bullets.`,
  },
  {
    category: 'Traction & Metrics',
    prompt: (bi) =>
      `For ${bi.companyName || bi.name || 'the company'}, list any public traction metrics (MRR/ARR, growth, users, customers), notable logos, and retention signals. If unknown, state unknown.`,
  },
  {
    category: 'Team & Execution',
    prompt: (bi) =>
      `For ${bi.companyName || bi.name || 'the company'}, summarize team highlights: founder backgrounds, team size signals, hiring momentum, and notable advisors.`,
  },
  {
    category: 'Product & Technology',
    prompt: (bi) =>
      `For ${bi.companyName || bi.name || 'the company'}, summarize product, tech stack hints, IP/moat, integrations, and ecosystem role.`,
  },
  {
    category: 'Fundraising & Capital',
    prompt: (bi) =>
      `For ${bi.companyName || bi.name || 'the company'}, summarize funding events, investors, stage alignment, and planned use of funds if known.`,
  },
  {
    category: 'Risks & Red Flags',
    prompt: (bi) =>
      `For ${bi.companyName || bi.name || 'the company'}, summarize any apparent risks: regulation, competition, dependency, market headwinds.`,
  },
];
 ...
 */
export async function fetchGptKnowledgeForCompany(
  businessInfo: BusinessInfo,
  signal?: AbortSignal,
): Promise<GptKnowledgeResult> {
