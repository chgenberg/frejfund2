...
async function embedTexts(texts: string[]): Promise<number[][]> {
  ...
  try {
    ...
    return resp.data.map(
      (d: any) => (d.embedding as unknown as number[]) || generateLocalEmbedding(''),
    );
  } catch {
    return texts.map((t) => generateLocalEmbedding(t));
  }
...
export async function indexContextForSession(
  sessionId: string,
  rawText: string,
  source?: { url?: string },
): Promise<number> {
...
export async function retrieveTopK(sessionId: string, query: string, k = 3): Promise<VectorItem[]> {
  if (backend === 'postgres') {
    ...
    const res = await db.query(
      'SELECT id, text, url, embedding FROM vector_items WHERE session_id = $1',
      [sessionId],
    );
    ...
    const scored = rows.map((r) => {
      ...
      return {
        it: { id: r.id, text: r.text, url: r.url || undefined, embedding: emb },
        score: cosineSimilarity(emb, qv),
      };
    });
