...
const FIXED_QUESTIONS: Record<string, string> = {
  business_idea:
    'Tell me about your business idea in a few sentences. What problem are you solving, and for whom?',
...
Respond with ONLY the question text (no preamble, no "Question:", just the question).`;
      ...
      const response = await this.getClient().chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert business advisor conducting investor due diligence.',
          },
  ...
  private inferQuestionArea(question: string, areasToExplore: string[]): string | null {
    ...
    if (lowerQ.includes('customer') || lowerQ.includes('market') || lowerQ.includes('acquisition'))
      return 'go_to_market';
    if (lowerQ.includes('product') || lowerQ.includes('feature') || lowerQ.includes('roadmap'))
      return 'product_roadmap';
    if (lowerQ.includes('valid') || lowerQ.includes('test') || lowerQ.includes('proof'))
    ...
    if (lowerQ.includes('regulat') || lowerQ.includes('legal') || lowerQ.includes('compliance'))
      return 'regulatory_considerations';
  ...
  private getFallbackQuestion(state: ConversationState): IntelligentQuestion {
    const fallbackQuestions: Record<string, string> = {
      ...
      key_risks:
        "What's the biggest risk to your business right now, and how are you addressing it?",
    ...
    try {
      const response = await this.getClient().chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a world-class business analyst preparing an investment memo.',
          },
          { role: 'user', content: prompt },
        ],
        temperature: 0.7,
