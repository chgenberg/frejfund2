...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!businessInfo)
      return NextResponse.json({ error: 'businessInfo required' }, { status: 400 });
...
Focus on: investment readiness, growth levers, numeric targets for 90 days, and one high-leverage question.`;
    ...
    if (!hasKey) {
      const fallback = {
        summary: `${businessInfo.name} in ${businessInfo.industry} at ${businessInfo.stage} stage.`,
        recommendations: ['Prioritize 90‑day plan', 'Talk to 10 customers', 'Define pricing'],
        questions: ['What is your next milestone?', 'Who is your ICP?', 'What is your pricing?'],
      };
