...
export async function POST(request: NextRequest) {
  try {
    const { message, businessInfo, conversationHistory, docContext, sessionId } =
      await request.json();
    ...
    if (sessionId) {
      try {
        const metricsResponse = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/metrics/update`,
          {
            method: 'POST',
        ...
        if (metricsResponse.ok) {
          ...
          if (metricsData.metricsFound) {
            ...
            console.log(
              `📊 Extracted ${extractedCount} metrics, re-analyzing ${metricsData.affectedDimensions?.length || 0} dimensions`,
            );
          }
        }
      } catch (error) {
    ...
    try {
      response = await aiAnalyzer.generateChatResponse(
        ...
        Array.isArray(conversationHistory)
          ? (conversationHistory as any[]).filter((m: any) => m?.type !== 'summary')
          : [],
        sessionId,
      );
    } catch (e) {
        ...
        try {
          ...
          const r = await client.chat.completions.create({
            ...
            messages: [
              {
                role: 'system',
                content: 'You are Freja. Be concise, professional, no emojis. Use markdown.',
              },
          ...
          response =
            ...
            'I had trouble connecting, but here is a brief answer.';
        } catch (e3) {
          console.error('Chat degraded fallback failed:', e3);
          return NextResponse.json({ error: 'Upstream temporary failure' }, { status: 502 });
        }
      }
    }
