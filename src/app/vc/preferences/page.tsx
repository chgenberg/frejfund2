...
export default function VCPreferencesPage() {
  ...
  const save = async () => {
    if (!vcEmail) {
      setStatus('Please login as VC first');
      return;
    }
    setStatus('Saving...');
    try {
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-8">
        ...
        <section>
          <div className="text-sm font-semibold text-black mb-2">
            Stages <span className="text-xs text-gray-500">(pick 1–3)</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {STAGES.map((s) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-full border ${stages.includes(s) ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-300 hover:border-black'}`}
              >
                {s}
              </button>
            ))}
        ...
        <section>
          <div className="text-sm font-semibold text-black mb-2">
            Industries <span className="text-xs text-gray-500">(max 5)</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {INDUSTRIES.map((s) => (
              <button
                key={s}
                onClick={() => toggle(industries, s, setIndustries)}
                className={`px-3 py-1.5 rounded-full border ${industries.includes(s) ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-300 hover:border-black'}`}
              >
                {s}
              </button>
            ))}
        ...
        <section>
          <div className="text-sm font-semibold text-black mb-2">
            Geography <span className="text-xs text-gray-500">(your focus)</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {GEOS.map((s) => (
              <button
                key={s}
                onClick={() => toggle(geographies, s, setGeographies)}
                className={`px-3 py-1.5 rounded-full border ${geographies.includes(s) ? 'bg-black text-white border-black' : 'bg-white text-gray-700 border-gray-300 hover:border-black'}`}
              >
                {s}
              </button>
            ))}
        ...
        <section className="grid md:grid-cols-2 gap-6">
          <div>
            <label className="text-sm text-gray-600">
              Minimum check size (USD) <span className="text-xs text-gray-400">(e.g. 250000)</span>
            </label>
            <input
              type="number"
              value={checkMin}
              onChange={(e) => setCheckMin(e.target.value === '' ? '' : Number(e.target.value))}
              className="w-full border border-gray-300 rounded-lg px-3 py-2"
              placeholder="250000"
            />
          </div>
          <div>
            <label className="text-sm text-gray-600">
              Maximum check size (USD) <span className="text-xs text-gray-400">(e.g. 2000000)</span>
            </label>
            <input
              type="number"
              value={checkMax}
              onChange={(e) => setCheckMax(e.target.value === '' ? '' : Number(e.target.value))}
              className="w-full border border-gray-300 rounded-lg px-3 py-2"
              placeholder="2000000"
            />
          </div>
        ...
        <section>
          ...
          <textarea
            ...
            rows={4}
            className="w-full border border-gray-300 rounded-lg px-3 py-2"
            placeholder="Brief outline: team, traction, vertical, go-to-market…"
          />
          <div className="text-[11px] text-gray-500 mt-1">
            Tip: The clearer your criteria, the better the matching (e.g. "B2B SaaS, seed-A, EU,
            &gt;$50k MRR, sales-led").
          </div>
        </section>
