...
export default function AdminDashboard() {
  ...
  const doBoost = async () => {
    if (!boostInvestorId || typeof boostRanking !== 'number') {
      setStatus('Provide investorId and ranking');
      return;
    }
    setStatus('Boosting...');
    try {
    ...
    <div className="min-h-screen bg-white">
      <header className="border-b border-gray-200 px-4 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <button onClick={load} className="px-4 py-2 bg-black text-white rounded-lg">
            Refresh
          </button>
        </div>
      </header>
        ...
        <section className="flex items-center justify-between">
          <div>
            ...
            <button
              onClick={doExport}
              disabled={exporting}
              className={`px-4 py-2 rounded-lg ${exporting ? 'bg-gray-200 text-gray-500' : 'bg-black text-white hover:bg-gray-800'}`}
            >
          ...
          <div className="border border-gray-200 p-4 rounded-lg">
            <div className="text-sm font-semibold text-black mb-2">
              Manual Boost (Investor Ranking)
            </div>
            <div className="flex items-center gap-2">
              <input
                value={boostInvestorId}
                onChange={(e) => setBoostInvestorId(e.target.value)}
              ...
              <input
                ...
                value={boostRanking}
                onChange={(e) =>
                  setBoostRanking(e.target.value === '' ? '' : Number(e.target.value))
                }
                placeholder="Ranking"
