...
interface KpiUploadModalProps { onClose: () => void; initialCsv?: string }

type KpiRow = {
  date: string; mrr: number; new_customers: number; churned_customers: number; arpa: number; cac: number; marketing_spend: number;
}
  ...
  useState(() => {
    if (initialCsv) {
      try {
        ...
        const data = (r.data as any[]).map((d:any)=>({
          date: String(d.date||''),
          mrr: Number(d.mrr||0), new_customers:Number(d.new_customers||0), churned_customers:Number(d.churned_customers||0), arpa:Number(d.arpa||0), cac:Number(d.cac||0), marketing_spend:Number(d.marketing_spend||0)
        })).filter((x:any)=>x.date);
        setRows(data);
      } catch {}
    }
      ...
      <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
        <motion.div className="bg-white rounded-2xl w-full max-w-3xl border border-gray-200 shadow-lg" initial={{scale:0.95,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.95,opacity:0}}>
          ...
          <div className="p-5 space-y-4">
            <input type="file" accept=".csv" onChange={(e)=>{const f=e.target.files?.[0]; if(f) parseFile(f);}} className="text-sm" />
            ...
            {rows.length>0 && (
              <div>
                ...
                <div className="flex justify-end mt-3 gap-2">
                  <button className="text-xs border border-gray-300 rounded px-2 py-1 hover:bg-gray-50" onClick={()=>downloadCSV(rows)}>Export CSV</button>
                  <button className="text-xs border border-gray-300 rounded px-2 py-1 hover:bg-gray-50" onClick={()=>downloadJSON(rows)}>Export JSON</button>
                </div>
                <div className="mt-3 max-h-56 overflow-y-auto border border-gray-200 rounded">
                  <table className="w-full text-xs">
                    <thead className="bg-gray-100 text-gray-700">
                      <tr>
                        {['date','mrr','new_customers','churned_customers','arpa','cac','marketing_spend'].map(h=> <th key={h} className="px-2 py-1 border-b text-left">{h}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {rows.map((r,i)=> (
...
function computeMetrics(rows: KpiRow[]){
  if(rows.length===0) return { lastMRR:0, avgChurn:0, avgCAC:0, avgARPA:0, ltv:0, paybackMonths:0, avgNew:0, avgChurnCount:0 };
  ...
  return { lastMRR, avgChurn: avgChurnRate, avgCAC, avgARPA, ltv, paybackMonths, avgNew, avgChurnCount };
}

function downloadCSV(rows: KpiRow[]){
  const headers = ['date','mrr','new_customers','churned_customers','arpa','cac','marketing_spend'];
  const table = [headers, ...rows.map(r=> headers.map(h=> (r as any)[h]))];
  const csv = table.map(r => r.map((x)=> '"'+String(x??'').replace(/"/g,'""')+'"').join(',')).join('\n');
  const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download='kpi.csv'; a.click(); URL.revokeObjectURL(url);
}

function downloadJSON(rows: KpiRow[]){
  const blob = new Blob([JSON.stringify(rows, null, 2)], { type: 'application/json' });
  const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download='kpi.json'; a.click(); URL.revokeObjectURL(url);
}
