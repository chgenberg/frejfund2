...
function parseInvestorsFromText(text) {
  ...
  const headers = [
    'sverige', 'sweden', 'vc-firma', 'fokus & styrka', 'fokus', 'styrka',
    'vad de letar efter', 'kontakt', 'webbplats', 'portföljbolag',
    'investeringsfokus', 'stage', 'checkstorlek', 'notable investments'
  ];
  ...
  while (i < lines.length) {
    ...
    if (looksLikeFirmName) {
      ...
      while (j < lines.length && j < i + 10) {
        ...
        if (nextLine.toLowerCase().includes('portfölj') || 
        ...
        // Collect description
        if (nextLine.length > 40 && nextLine.length < 800 && 
...
function looksLikeHeader(text) {
  const headers = [
    'fokus', 'styrka', 'letar efter', 'kontakt', 'webbplats',
    'portföljbolag', 'investeringsfokus', 'checkstorlek'
  ];
...
function extractFinancials(text, investor) {
  // Extract fund size
  const moneyMatch = text.match(/[€\$]\s*(\d+(?:[,\s]\d+)?(?:\.\d+)?)\s*(miljard|miljon|billion|million|mdr|M|B)/i);
  if (moneyMatch) {
